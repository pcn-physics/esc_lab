'''
Most datasets you will be working with will be stored as data frames. 
By the end of this chapter, you will be able to create a data frame, select interesting parts of a data frame, 
and order a data frame according to certain variables.
'''
...
When you work with (extremely) large data sets and data frames, 
...
#Wonderful! So, what do we have in this data set? For example, hp represents the car\'s horsepower;
the Datsun has the lowest horse power of the 6 cars that are displayed. For a full overview of the variables\' meaning,
type ?mtcars in the console and read the help page.
'''
...
name, type, diameter, rotation and rings; these vectors have already been coded up on the right. 
...
the vectors you pass should also have the same length. 
...
The planets_df data frame should have 8 observations and 5 variables. It has been made available in the workspace, 
...
and you managed to pull out all relevant data. Pretty awesome! By now, NASA is probably already flirting with your CV ;-).
...
Exercise
Sorting
Making and creating rankings is one of mankind\'s favorite affairs. These rankings can be useful (best universities in the world),
