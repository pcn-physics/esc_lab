...
> 
> # Popular days
> popularDaysLinkedin <- linkedin > 15
> popularDaysLinkedin
[1]  TRUE FALSE FALSE FALSE FALSE  TRUE FALSE
> popularDaysFacebook <- facebook > 15
> popularDaysFacebook
[1]  TRUE FALSE FALSE  TRUE FALSE FALSE FALSE
> 
> # Quiet days
> quietDaysLinkedin <- linkedin <= 5
> quietDaysLinkedin
[1] FALSE FALSE FALSE  TRUE  TRUE FALSE FALSE
> quietDaysFacebook <- facebook <= 5
> quietDaysFacebook
...
'''
Logical Operator
AND: &; E.g. TRUE & TRUE returns TRUE; 
x <- 12
x > 5 & x < 15 return TRUE
OR: |
NOT: !
is.numeric(5) return TRUE
!is.numeric(5) returns false
is.numeric("hello") returns false

& vs &&; #Similar for | vs ||
c(TRUE, TRUE) & c(TRUE, FALSE) return TRUE FALSE
c(TRUE, TRUE) && c(TRUE, FALSE) returns TRUE #Only examines first element
'''
...
its first and second row correspond to the linkedin and facebook vectors, respectively. 
...
'''
Reverse the result: !
On top of the & and | operators, you also learned about the ! operator, which negates a logical value. 
To refresh your memory, here are some R expressions that use !. They all evaluate to FALSE:
...
You stored the results in a data frame called li_df. This data frame is available in the workspace; 
type li_df in the console to check it out.
...
The medium variable gives information about the social website; the num_views variable denotes the actual number of views that 
particular medium had on the last day of your recordings. Both these variables have already been defined in the editor.
...
You can add as many else if statements as you like. Keep in mind that R ignores the remainder of the control structure once a 
ondition has been found that is TRUE and the corresponding expressions have been executed. 
...
Again, It's important that the else if keywords comes on the same line as the closing bracket of the previous part of the 
control construct!
...
In both cases, the existing code should be extended in the else if statement. No existing code should be modified.
'''
...
Because R abandons the control flow as soon as it finds a condition that is met,
you can simplify the condition for the else if part in the second construct to num_views > 10
'''
...
Exercise
Take control!
In this exercise, you will combine everything that you've learned so far: relational operators, 
logical operators and control constructs. You'll need it all!

In the editor, we've coded two values beforehand: li and fb, 
denoting the number of profile views your LinkedIn and Facebook profile had on the last day of recordings. 
